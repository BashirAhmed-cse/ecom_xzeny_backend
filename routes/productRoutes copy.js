...
router.post(
  ...
  authorizeAdmin,
  (req, res, next) => {
    console.log("✅ Authenticated admin. Proceeding to multer...");
    next();
  },
  ...
  (req, res, next) => {
    console.log("🖼️ Multer processed files:", req.files?.length || 0);

    // ✅ UPDATED: Log all files for debugging
    if (req.files && req.files.length > 0) {
      console.log("📁 All uploaded files:");
      req.files.forEach((file, index) => {
        console.log(`  ${index + 1}. ${file.fieldname} - ${file.originalname}`);
      });
    }
    ...
    for (const file of req.files || []) {
      ...
      if (!isValidField) {
        console.log(`⚠️ Invalid field detected: ${file.fieldname}`);
